  ...
  return (
    ...
    >
      {isEditModeEnabled && showTitle && (
        <div className="grid-item__title">Drag here</div>
      )}
      <div className="grid-item__body">{item}</div>
    </div>
  );
}
  ...
  return (
    <div className="edit-mode-control">
      <button
        onClick={(e) => {
          e.preventDefault();
          onEditModeToggle(!isEditModeEnabled);
        }}
      >
        {isEditModeEnabled ? "Turn Edit Mode Off" : "Turn Edit Mode On"}
      </button>
    </div>
  );
}
...
function filterObjectKeys(item, keys) {
  return Object.entries(item).reduce(
    (newItem, [key, val]) =>
      keys.includes(key) ? { [key]: val, ...newItem } : newItem,
    {}
  );
}
...
export function GameInterface({ children }) {
  ...
  const layouts = {
    lg: [
      { i: "1", y: 0, x: 0, h: 1, w: 12 },
      { i: "2", y: 1, x: 0, h: 9, w: 6 },
      { i: "3", y: 1, x: 6, h: 12, w: 6 },
      { i: "4", y: 13, x: 0, h: 5, w: 12 },
      { i: "5", y: 10, x: 0, h: 3, w: 6 },
    ],
    // sm: [
    //   { i: "1", x: 0, y: 0, w: 1, h: 1 },
    //   { i: "2", x: 1, y: 0, w: 2, h: 2 },
  ...
  function handleLayoutChange(layout) {
    ...
    const prettyLayout = layout.map((block) =>
      filterObjectKeys(block, keysToPrint)
    );
    ...
    <div>
      ...
      >
        ...
        {children.slice().map((child, i) => (
          <div
            className={"grid-item" + (isEditModeEnabled ? " editable" : "")}
            key={i + 1}
          >
            <GridItemWrapper
              isEditModeEnabled={isEditModeEnabled}
